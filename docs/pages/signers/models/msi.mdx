...
- `HardwareSigner`: A signer that uses hardware-based security features (e.g., Secure Enclave, Android Keystore) to sign payloads.

## Properties

- `dummySignature`: A valid signature that can be used for testing purposes, specifically to simulate user operations on the entrypoint. Implementations must specify a dummy signature that matches their transaction signature standard.
...
The `MultiSignerInterface` provides a standardized way to interact with different types of signers, including hardware-based signers like the `HardwareSigner`, making it easier to work with various signing mechanisms in the Ethereum ecosystem.
