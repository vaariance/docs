...
```dart
import 'package:variance/variance.dart';
...
The `PassKeySigner` conforms to the multi-signer interface and allows you to sign payloads using your device's passkeys. It falls under the `secp256r1` category and can be verified on-chain using the `P256Verifier` precompile:
...
The `HardwareSigner` provides a high-level abstraction for the `SecureP256` class to conform to the multi-signer interface. However, you can access the `SecureP256` class directly, which supports additional operations like encrypt, decrypt, and verify.
...
```dart
await wallet.send(
  EthereumAddress.fromHex("0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789"), // Recipient address
